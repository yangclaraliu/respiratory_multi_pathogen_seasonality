...
We assembled paired long-term surveillance data for RSV and seasonal influenza in mainland China based on a unified inclusion strategy.
We strictly required continuous, year-indexed time series (excluding aggregated seasonal averages) with a minimum duration of five consecutive years, collected at the city level with monthly or weekly resolution.
RSV datasets were identified by screening the extraction database of a recent systematic review \cite{guo2024respiratory}.
For the identified cities with eligible RSV data, we assembled matching influenza time series using a hybrid approach: we first screened major influenza systematic reviews\cite{Diamond2022_ChinaSeasonality_BMCMed,Li2021_ChinaBurden_SciRep}, and subsequently conducted targeted PubMed searches for specific cities where review data were unavailable.
When multiple datasets existed for a city, we prioritized those with longer duration and laboratory confirmation.
Data presented in figures were digitized using WebPlotDigitizer.
For analysis, raw surveillance metrics were natural log-transformed to stabilize variance, with zeros replaced by a small constant.
Detailed search strategies, data sources, and surveillance definitions are provided in the Supplementary Material.
