LL_flu %>% 
  ...
  pivot_longer(cols = starts_with("contribution_cycle")) %>%
...
ggplot(p_tab, aes(x = 1, y =  value, color = name, fill = name)) +
  ...
  labs(color = "Component", fill = "Component", 
       x = "City",
       y = expression(frac("variance(component)", "variance(observed)"))) 


LL_flu %>% 
  map(arrange, desc(distribution_specific_AIC)) %>% 
  map(mutate,
      rank_ll = 1:n(),
      n = max(distribution_specific_AIC),
      top_np = round(n*0.1)) %>% 
  map(dplyr::filter,
      rank_ll <= top_np) %>% 
  bind_rows() %>% 
